...
router.get('/', [
  ...
  query('category').optional().isIn(['prepared_food', 'baked_goods', 'produce', 'packaged_food', 'beverages']),
  ...
  try {
    ...
    const { lat, lng, radius = 10, category, search, limit = 20, page = 1 } = req.query
    ...
    const enrichedListings = listings.map(listing => ({
      ...
      timeRemaining: listing.getTimeRemaining ? listing.getTimeRemaining() : null,
      urgencyLevel: listing.getUrgencyLevel ? listing.getUrgencyLevel() : null,
...
router.post('/', [
  ...
  body('category').isIn(['prepared_food', 'baked_goods', 'produce', 'packaged_food', 'beverages']),
  body('quantity').trim().isLength({ min: 1, max: 100 }).withMessage('Quantity is required'),
  ...
  body('location.address').trim().isLength({ min: 5, max: 300 }).withMessage('Address must be 5-300 characters')
], async (req, res) => {
  try {
    const errors = validationResult(req)
    if (!errors.isEmpty()) {
...
router.put('/:id', [
  ...
  try {
    ...
    const allowedUpdates = ['title', 'description', 'quantity', 'expiryTime', 'pickupInstructions', 'allergens']
    const updates = {}
